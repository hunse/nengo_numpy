...
class Connection(object):
    ...
    def build(self, dtype):
        ...
        if isinstance(self.pre, list):
            ...
            if self.neuron_space and all_ensembles:
                ...
                self.pre_dims = sum(input.size for input in self.input)
            else:
        ...
        if self.output is None:
            self.output = np.zeros(self.post_dims, dtype=dtype)
    ...
    def tick(self, dt):
        ...
        if self.neuron_space:
            if self.weights is not None:
                value = np.dot(self.input, self.weights)
            else:
                value = self.input
        elif isinstance(self.input, list):
            ### list of populations to decode
            if self.decoders is not None:
                value = np.asarray([np.dot(input, dec) for input, dec in
                                    zip(self.input, self.decoders)]).flatten()
            else:
                value = np.asarray(self.input).flatten()

            if self.transform is not None:
                value = np.dot(value, self.transform)
        else:
            ...
            if self.decoders is not None and self.transform is not None:
                value = np.dot(np.dot(self.input, self.decoders), self.transform)
            elif self.decoders is not None:
                value = np.dot(self.input, self.decoders)
            elif self.transform is not None:
                value = np.dot(self.input, self.transform)
            else:
                value = self.input
        ...
        if self.filter > 0:
            ...
            if self.spiking_input:
                self.output[:] = decay*self.output + ((1-decay)/dt)*value
            else:
                self.output[:] = decay*self.output + (1-decay)*value
        else:
            self.output[:] = value

        ### add output to post population
        # self.post_input[:] += self.output
